    ...
    if (isset($_POST['METHOD']) && $_POST['METHOD'] == 'LOGIN_USER') {
        ...
        if ($resultadoEmail) {
            ...
            if ($userEmail) {
                ...
                // Verificar si la cuenta está pendiente de validación
                if ($userEmail['is_activate'] == 2) {
                    error_log("❌ AUTH FAILED: Cuenta pendiente de validación - " . $email);
                    echo json_encode(array("error" => "Esta cuenta aún está pendiente para validar."));
                    header("HTTP/1.1 403 Forbidden");
                    exit();
                }
